...
class SalesforceBulk(object):
    ...
    def __init__(self, sessionId=None, host=None, username=None, password=None, token=None, sandbox=False,
        ...
        if not sessionId:
            sessionId, endpoint = SalesforceBulk.login_to_salesforce(
                username, password, token, sandbox)
    ...
    @staticmethod
    def login_to_salesforce(username, password, token, sandbox=False):
        env_vars = (
        ...
        packet = salesforce_oauth_request.login(
            username=username, password=password, sandbox=sandbox, token=token)
